...
<c:if test="${sessionScope['loginUser'].viewRecentTaskAllowed}">
 									<a href="javascript:void(0)" class="dropdown-item" id="recent-task-btn">
 										<i class="fa-light fa-clipboard-list"></i> &nbsp; 최근 기기 작업</a>
</c:if>
									<a href="javascript:void(0)" class="dropdown-item" id="pwd-update-btn">
										<i class="fa-light fa-key"></i> &nbsp; 패스워드 변경</a>
									<div class="dropdown-divider"></div>
									<a href="javascript:void(0)" class="dropdown-item" id="logout-btn">
										<i class="fa-light fa-sign-out-alt text-danger"></i> &nbsp; 로그아웃</a>
								</div>
							</div>
						</div>
					</nav>
...
<c:if test="${isViewSwitcherMode && not empty sessionScope['loginUser'] && sessionScope['loginUser'].viewSwitcherShown}">
								...
								<div class="btn-group pl-2">
									<button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown">
										<span class="fa-duotone fa-filter" style="--fa-secondary-color: mediumseagreen; --fa-secondary-opacity: 1.0;"></span>
